...
class Results implements \SeekableIterator, \Countable {
  ...
  public function rewind() { if ($this->rows) $this->seek(0); }
  ...
  public function current() {
    if (!$this->current) {
      ...
      $this->current = mysql_fetch_assoc($this->mysql_rs);
      $this->ahead = True;
    }

    // Don't return an array if theres only one item
    if (count($this->current) == 1) return array_first($this->current);
  ...
  public function get() {
    ...
    if (count($row) == 1) return array_first($row);
    else return $row;
  }
  ...
  function asArray() {
	...
	function asMap() {
		$a = array();
		while ($r = $this->getRow()) {
			if (count($r) == 1) $a[$r[0]] = $r[0];
			elseif (count($r) == 2) $a[$r[0]] = $r[1];
			else $a[$r[0]] = array_slice($r,1);
		}
		return $a;	
	}
	function asCSV($seperator=',') {
		$data = "";
		while ($row = $this->getRow()) {
			$data .= implode($seperator, $row)."\n";
		}
		return $data;
	}
}
